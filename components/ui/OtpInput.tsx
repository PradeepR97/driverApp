...
type Props = {
  ...
  disabled?: boolean;
  /** When true, boxes use danger border until cleared. */
  hasError?: boolean;
  /** Shake entire row (e.g. after failed verify). */
  shakeTrigger?: number;
...
export function OtpInput({
  ...
  const setDigit = (index: number, char: string) => {
    ...
    next[index] = c;
    onChange(next);
    if (c && index < length - 1) focusInput(index + 1);
  ...
  const onKeyPress = (index: number, key: string) => {
    if (key !== 'Backspace' || disabled) return;
    const next = [...value];
    if (next[index]) {
      next[index] = '';
      onChange(next);
