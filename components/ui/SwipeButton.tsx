...
type Variant = 'primary' | 'danger' | 'offline' | 'indigo' | 'success';
...
type Props = {
  ...
  variant?: Variant;
  testID?: string;
  /** Mint track with a stronger primary border (home “go online”). */
  trackOutlined?: boolean;
  /** Bump to force a full reset (knob position + gesture) when the same component tree position is reused. */
  resetKey?: number | string;
};

const variantStyles: Record<
  Variant,
  { track: object; fill: object; knob: object; text: object; icon: string }
> = {
  primary: {
  ...
  indigo: {
    ...
    fill: { backgroundColor: 'rgba(55, 48, 163, 0.28)' },
    knob: { backgroundColor: '#3730A3' },
    text: { color: '#1E1B4B' },
    icon: Colors.white,
  },
  success: {
    track: { backgroundColor: '#DCFCE7' },
    fill: { backgroundColor: 'rgba(22, 163, 74, 0.28)' },
    knob: { backgroundColor: '#16A34A' },
    text: { color: '#166534' },
    icon: Colors.white,
  },
};
