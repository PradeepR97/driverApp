...
/**
 * Fleet API base (no trailing slash). Default from `EXPO_PUBLIC_API_URL` in `.env`;
 * in development, the app can persist an override (see `lib/api-base-url.ts`).
 *
 ...
 */
export const AUTH_USER_TYPE = "DRIVER";
...
function websocketBaseUrlFromApiBase(apiBase) {
  try {
    const raw = apiBase.startsWith("https") ? apiBase : `https://${apiBase}`;
    const u = new URL(raw);
...
export function buildDriverWebSocketConnection(accessToken) {
  ...
  const url = websocketBaseUrlFromApiBase(getApiBaseUrl());
  if (!url) {
    if (__DEV__) {
      console.warn(
        "[driver-ws] could not derive WebSocket URL from API base:",
        getApiBaseUrl(),
        "— set EXPO_PUBLIC_WS_URL or fix the API URL",
      );
    }
    return null;
  }
...
export function formatWsConnectMessage(accessToken) {
 ...
 */
export function getDirectS3ClientConfig() {
  const region =
    (typeof process !== "undefined" &&
  ...
  const aclRaw =
    ...
    "";
  const objectAcl = aclRaw === "public-read" ? "public-read" : undefined;
...
export function publicUrlForS3Key(cfg, key) {
