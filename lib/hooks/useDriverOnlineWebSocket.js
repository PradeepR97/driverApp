...
import { buildDriverWebSocketConnection, DRIVER_LOCATION_PUSH_INTERVAL_MS, formatWsConnectMessage, } from '@/config/appConfig';
import { useDriverStore } from '@/lib/driver-store';
import { formatDriverLocationWebSocketMessage, getDriverCoordsForLocationPing, } from '@/services/location/core/driver-coords';
import { playNewOrderAlertSound } from '@/services/notifications/orderAlertSound';
import { parseDriverWsMessage } from '@/services/sockets/realtime/driver-ws-incoming';
import { connectDriverWebSocketWithRetry, } from '@/services/sockets/realtime/driver-websocket';
 ...
 */
export function useDriverOnlineWebSocket(isOnline) {
    ...
    useEffect(() => {
        ...
        const pushLocationOnce = async () => {
            ...
            try {
                ...
                if (!coords) {
                    if (__DEV__) {
                        devLog.wsWarn('[driver-ws] location skipped (no permission or GPS error)');
                    }
                    return;
                }
            ...
            handlers: {
                ...
                onMessage: (data) => {
                    ...
                    if (parsed.kind === 'ERROR' && __DEV__) {
                        devLog.wsError('[driver-ws] ERROR frame', parsed.message);
                    }
                    if (parsed.kind === 'ACK' && __DEV__ && parsed.message === 'CONNECTED') {
                        devLog.wsOpen('[driver-ws] ACK CONNECTED');
                    }
                    if (parsed.kind === 'LOCATION_ACK' && __DEV__) {
                        devLog.wsOpen('[driver-ws] LOCATION_RECEIVED');
                    }
                },
            },
        });
