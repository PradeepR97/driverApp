...
export type ShakeOptions = {
  /** Total duration in ms. Recommended: 300–600 */
  durationMs?: number;
  /** Max translation in px. Recommended: 8–12 */
  amplitude?: number;
};

export function useShakeAnimation(opts: ShakeOptions = {}) {
  ...
  const shake = () => {
    ...
    const t = Math.max(
      ...
      Math.min(SHAKE_DURATION_MS_MAX, durationMs),
    );
