...
function toState(status?: string, canAskAgain?: boolean): NotificationPermissionState {
...
export async function getNotificationPermissionStateIfAvailable(): Promise<NotificationPermissionState> {
  if (Constants.executionEnvironment === 'storeClient') {
    return 'unavailable';
  }
  try {
...
export async function requestNotificationPermissionStateIfAvailable(): Promise<NotificationPermissionState> {
  if (Constants.executionEnvironment === 'storeClient') {
    return 'unavailable';
  }
  try {
