...
function normalizeDeviceLanguageTag(tag) {
...
export async function setStoredAppLanguage(lang) {
    try {
...
export async function ensureI18nInitialized() {
    ...
    initPromise = (async () => {
        ...
        const initialLanguage = stored ?? device ?? 'en';
        ...
        await setStoredAppLanguage(i18n.language);
    })();
    return initPromise;
}
/**
 ...
 */
export function applyDeviceLanguageInMemoryOnly() {
    if (!i18n.isInitialized)
        return;
    const device = normalizeDeviceLanguageTag(Localization.getLocales?.()[0]?.languageTag);
